...
function Forum() {
  ...
  return (
    <div className="flex flex-col w-full p-4 gap-6 bg-gray-100">
      ...
      <form onSubmit={handleCreateQuestion} className="flex flex-col gap-4 bg-white shadow-md rounded-lg p-6 border-2 border-green-500 mb-8">
        ...
        <button type="submit" className="w-full bg-green-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-green-700 transition">Publicar pregunta</button>
      </form>

      <h2 className="text-2xl font-semibold text-green-700 mb-4">Preguntas recientes</h2>
      {questions.map((q) => (
        <div key={q.id} className="bg-white shadow-md rounded-lg p-6 mb-4 border-2 border-green-500">
